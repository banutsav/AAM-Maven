...
import static edu.uga.ovpr.aam.manager.FindAssocDAO.parseFSCodes;
 ...
 * @author submyers
 */
public class ManAssociateDAO {

    /*
        get data for an existing associate
    */
    
    public static void GetAssociateData(ManAssociateForm manAssociateForm, String cannum, String role) throws Exception {
        Connection conn = null;
        HashMap<String, Object> AssociateData = new HashMap<String, Object>();
        try {
            conn = Static.getConnection("jdbc/paul");

            final Statement getAssociateDataStat = conn.createStatement();
            final Statement getPurposesStat = conn.createStatement();

            final String getAssociateDataQuery = "SELECT DISTINCT"
                    + " `a`.`ID` AS `ID`"
                    ...
                    + " WHERE"
                    + " `apl`.`AFFILIATE_PURPOSE_ID` = '" + Constants.associate_code + "' AND "
                    + " `a`.`ID`='" + GeneralDAO.CleanMySQLString(manAssociateForm.getassocId()) + "'";
            

            //System.out.println("Get Aff Data Query:\n" + getAffDataQuery + "\n");
            
            final ResultSet getAssociateDataRs = getAssociateDataStat.executeQuery(getAssociateDataQuery);
            if (getAssociateDataRs.next()) {
                ...
                getPurposesQuery += " FROM"
                        ...
                        + " ON `apl`.`AFFILIATE_ID`='" + getAssociateDataRs.getString("ID") + "'"
                ...
                AssociateData.put("ID", getAssociateDataRs.getString("ID"));
                AssociateData.put("FIRSTNAME", getAssociateDataRs.getString("FIRSTNAME"));
                AssociateData.put("MIDDLENAME", getAssociateDataRs.getString("MIDDLENAME"));
                AssociateData.put("LASTNAME", getAssociateDataRs.getString("LASTNAME"));
                AssociateData.put("OVPRID", getAssociateDataRs.getString("OVPRID"));
                AssociateData.put("EMAIL", getAssociateDataRs.getString("EMAIL"));
                AssociateData.put("PSEUDOCAN", getAssociateDataRs.getString("PSEUDOCAN"));
                AssociateData.put("TITLE", getAssociateDataRs.getString("TITLE"));
                AssociateData.put("DEPARTMENT", getAssociateDataRs.getString("DEPARTMENT"));
                AssociateData.put("ORGANIZATION", getAssociateDataRs.getString("ORGANIZATION"));
                AssociateData.put("PHONE", getAssociateDataRs.getString("PHONE"));
                AssociateData.put("ACTIVE", getAssociateDataRs.getString("ACTIVE"));
                //affData.put("INSTITUTION_COMPANY", getAffDataRs.getString("INSTITUTION_COMPANY"));
                AssociateData.put("ADDR_1", getAssociateDataRs.getString("ADDR_1"));
                AssociateData.put("ADDR_2", getAssociateDataRs.getString("ADDR_2"));
                AssociateData.put("CITY", getAssociateDataRs.getString("CITY"));
                AssociateData.put("STATE", getAssociateDataRs.getString("STATE"));
                AssociateData.put("COUNTRY", getAssociateDataRs.getString("COUNTRY"));
                AssociateData.put("ZIP", getAssociateDataRs.getString("ZIP"));
                AssociateData.put("BUSINESS_PHONE_NUM", getAssociateDataRs.getString("BUSINESS_PHONE_NUM"));
                //affData.put("ALT_PHONE_NUM", getAffDataRs.getString("ALT_PHONE_NUM"));
                AssociateData.put("FAX_NUMBER", getAssociateDataRs.getString("FAX_NUMBER"));
                //affData.put("EXPIRATION_DATE", getAffDataRs.getString("EXPIRATION_DATE").replaceFirst(":\\d{2}(\\.\\d){0,1}$", ""));
                AssociateData.put("COMMENTS", getAssociateDataRs.getString("COMMENTS"));
                
                
                AssociateData.put("IS_ENROLLED", getAssociateDataRs.getString("IS_ENROLLED"));
                AssociateData.put("UGA_HR_STATUS", getAssociateDataRs.getString("UGA_HR_STATUS"));
               
                
                AssociateData.put("FSCODES", FindAssocDAO.parseFSCodes(getAssociateDataRs.getString("FSCODES")));
                
                AssociateData.put("purposeDetails", purpose);
            }
            manAssociateForm.setassociate(AssociateData);
        } catch (Exception ex) {
            throw ex;
        } finally {
            if (conn != null) {
    ...
    /*
        update the expiration date for an existing associate
    */
    
    public static String UpdateAssociateExpirationDate(ManAssociateForm manAssociateForm) throws Exception {
        Connection conn = null;
        ...
        try {
            ...
            final String updateAffPurposeCmd = "UPDATE"
                    ...
                    + " SET"
                    + " `EXPIRATION_DATE`=DATE_ADD(NOW(), INTERVAL " + GeneralDAO.CleanMySQLString(manAssociateForm.getMonthCount()) + " MONTH)"
                    + ", `EXPIRES`='1'"
                    + " WHERE"
                    + " `AFFILIATE_PURPOSE_ID`='" + Constants.associate_code + "'"
                    + " AND `AFFILIATE_ID`='" + GeneralDAO.CleanMySQLString(manAssociateForm.getassocId()) + "'";
    ...
    /*
        set associate expiration date to never expires
    */
    
    public static void unexpireAssociatePurpose(ManAssociateForm manAssociateForm) throws Exception {
        Connection conn = null;
        Statement commitStat = null;
        try {
            conn = Static.getConnection("jdbc/paul");
            ...
            final String unexpirePurposeCmd = "UPDATE"
                    ...
                    + " `EXPIRES`='0'"
                    + " WHERE"
                    + " `AFFILIATE_ID`='" + manAssociateForm.getassocId() + "'"
